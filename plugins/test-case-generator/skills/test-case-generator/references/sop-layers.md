...
> **每个 sub-bullet 生成一条独立用例，不合并**。例如"分页功能"下的"上一页/下一页/跳页/每页条数切换/首页/末页/边界页码"必须拆成 7 条独立用例（详见 Writing Guidelines 颗粒度原则）。
...
> **每个 sub-bullet 生成一条独立用例，不合并**。例如"关键字搜索"下的"输入有效关键字 / 输入无效关键字 / 不输入直接搜索 / 输入特殊字符 / 输入超长字符串"必须拆成 5 条独立用例（ZYCZXZ_006 反例就是把这 5 个塞进 1 条用例，这是**错的**）。详见 Writing Guidelines 颗粒度原则。
...
**核心原则**: 遍历需求字段说明表中的每个字段, 根据字段类型生成对应的验证用例。**每个字段的每一条验证规则 = 1 条独立用例**（不合并）。
...
> **每个 sub-bullet 生成一条独立用例，不合并**。例如"审批流程"的"提交 / 通过 / 驳回 / 撤回"必须拆成 4 条独立用例（详见 Writing Guidelines 颗粒度原则）。
...
- 用例名称要具体, 体现测试的核心内容，便于 Playwright 自动化按名称定位（如 `"企业全称-必填校验"` 而非 `"企业全称字段校验"`）
...
- 本 SOP 中每一层（L0-L6）列出的每个子项（sub-bullet）对应**一条独立用例**，**不合并**为多步骤用例
- 每个字段的每一条验证规则 = 1 条独立用例（**不合并**同类型同规则字段）
- 每个下拉选项 = 1 条独立用例（**不做**"首末中"精简）
- 每个交互场景 = 1 条独立用例

**原因**：生成的用例同时是手工测试依据和 Playwright 自动化测试的参考。自动化引用时，1 条用例对应 1 个 test function / 1 个核心 assertion，便于失败定位和报告。

**唯一例外 — 多步骤不等于多检查点**：

- ✅ 正确：`steps="1.点击搜索框\n2.输入'测试A'\n3.点击查询按钮"`, `expected="1.搜索框获得焦点\n2.输入内容显示\n3.显示匹配结果"` 是 1 条用例，因为它只测试"用有效关键字搜索"这 1 个功能点，只是 operation 有多步
- ❌ 错误：`steps="1.输入完整企业全称\n2.输入企业简称\n3.输入空格\n4.输入特殊字符\n5.输入超长字符串"`, `expected="1.显示匹配记录\n2.显示包含简称的记录\n3.显示全部记录\n4.显示无记录\n5.显示过长提示"` 是**错误的**，每一步是一个独立的检查点，应拆分为 5 条独立用例，每条 `case_name` 形如 `"关键字搜索-完整企业全称"`、`"关键字搜索-企业简称"` 等
