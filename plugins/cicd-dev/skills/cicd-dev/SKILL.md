...
name: cicd-dev
description: >-
  CI/CD 流水线生成器：从项目技术栈信号自动生成 GitHub Actions / GitLab CI 流水线 YAML，
  包含 lint / test / build / deploy 分阶段、缓存策略与矩阵构建。
  Use when the user says "生成 CI 流水线", "GitHub Actions 配置", "GitLab CI",
  "pipeline", "ci/cd", "自动化流水线", "bootstrap ci", "stack detection",
  "检测技术栈", "加一个 staging 部署", or wants to create / migrate / audit a CI
  pipeline. Includes progressive rollout guidance (CI-only → staging → prod with
  manual gate) and platform decision notes.

  Not for: 部署到具体云平台（AWS SDK / GCP SDK / Azure CLI 操作），基础设施 IaC
  （Terraform / Pulumi / CloudFormation），应用级代码实现。
license: MIT
metadata:
  version: 1.0.0
  upstream_author: Alireza Rezvani
  upstream_source: https://github.com/alirezarezvani/claude-skills/tree/main/engineering/ci-cd-pipeline-builder
